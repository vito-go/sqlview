...
### UI 特性 (v0.1.0+)
- 🎨 **现代化标签页界面** - SQL 查询、模式浏览器、图表
- 📋 **模式浏览器** - 全面的数据库结构查看器，带统计信息
- 📊 **数据可视化** - 交互式图表（柱状图、折线图、饼图、环形图）
- ✨ **SQL 语法高亮** - 由 CodeMirror 驱动，支持多种主题
- 📜 **查询历史** - 本地存储最近 100 条查询
- 🌓 **深色/浅色主题** - 主题切换，带持久化偏好
- 💾 **多格式导出** - 导出为 CSV 或 JSON（客户端实现）
- 🎯 **智能右键菜单** - 快速访问常用操作
- 🔍 **可折叠侧边栏** - 需要时获得更多空间
- 📱 **响应式设计** - 在所有屏幕尺寸上都很美观
- 🖱️ **交互式结果表格** - 点击任意单元格查看完整内容并支持复制
- 📅 **时间聚合** - 按天、周、月或年对日期/时间列进行分组（支持 PostgreSQL、MySQL、SQLite）
